<div {{ $attributes->merge(['class' => '']) }}>
    <span class="{{ empty($errors->all()) ? 'js-contact-up' : '' }} mb-4 block text-3xl font-semibold">
        {{ __('message.contact.info.title') }}
    </span>
    <div class="{{ empty($errors->all()) ? 'js-contact-up' : '' }} flex flex-col gap-8 border-b-2 pb-4">
        <div class="{{ empty($errors->all()) ? 'js-contact-down' : '' }} flex gap-1">
    ...
    <div class="flex flex-col gap-4 pb-4">
        <span class="{{ empty($errors->all()) ? 'js-contact-right' : '' }} text-xl font-semibold">
            {{ __('message.contact.info.network') }}
        </span>
        <div class="flex">
            <div class="{{ empty($errors->all()) ? 'js-contact-right' : '' }} flex basis-16">
